...
export async function POST(request) {
  try {
    ...
    if (!alumnoId || !horarioId || !fecha) {
      return Response.json({ success: false, error: "Todos los campos son requeridos: alumnoId, horarioId, fecha" }, { status: 400 });
    }
    ...
    if (!horario) {
      return Response.json({ success: false, error: "El horario no existe" }, { status: 404 });
    }

    // Verificar que el alumno está asignado al curso
    const asignacion = await prisma.asignacion.findFirst({
      where: { alumnoId: parseInt(alumnoId), cursoId: horario.cursoId },
    });
    if (!asignacion) {
      return Response.json({ success: false, error: `El alumno no está asignado al curso ${horario.curso.nombre}` }, { status: 409 });
    }

    // Verificar asistencia duplicada en la misma fecha
    const asistenciaExiste = await prisma.asistencia.findUnique({
      where: { alumnoId_horarioId_fecha: { alumnoId: parseInt(alumnoId), horarioId: parseInt(horarioId), fecha: new Date(fecha) } },
    });
    if (asistenciaExiste) {
      return Response.json({ success: false, error: "Ya existe una asistencia registrada para este alumno en esta fecha y horario" }, { status: 409 });
    }
